...
// Ensure MediaPipe and Soundfont types are available globally
...
declare var Soundfont: {
    instrument: (audioContext: AudioContext, instrumentName: string, options?: any) => Promise<any>;
};
...
const leftInstrumentSelect = document.getElementById('left-instrument-select') as HTMLSelectElement; // Physical Left Hand (circle, screen right)
const rightInstrumentSelect = document.getElementById('right-instrument-select') as HTMLSelectElement; // Physical Right Hand (triangle, screen left)
...
const leftCustomSampleInput = document.getElementById('left-custom-sample-input') as HTMLInputElement;
const rightCustomSampleInput = document.getElementById('right-custom-sample-input') as HTMLInputElement;
const leftCustomSampleNameDisplay = document.getElementById('left-custom-sample-name-display') as HTMLSpanElement;
const rightCustomSampleNameDisplay = document.getElementById('right-custom-sample-name-display') as HTMLSpanElement;
...
const modalTitleElement = document.getElementById('modal-title-dynamic') as HTMLHeadingElement;
const thereminDescriptionDisplay = document.getElementById('theremin-description-display') as HTMLDivElement;
const tapPadDescriptionDisplay = document.getElementById('tappad-description-display') as HTMLDivElement;
const fingerKeysDescriptionDisplay = document.getElementById('fingerkeys-description-display') as HTMLDivElement;
...
let leftOscillator: OscillatorNode | null = null; // Path A: Controlled by Physical Right Hand
let leftOscillatorGainNode: GainNode | null = null; // Path A
let rightOscillator: OscillatorNode | null = null; // Path B: Controlled by Physical Left Hand
let rightOscillatorGainNode: GainNode | null = null; // Path B

// For SoundFont types
type SoundFontInstrumentName = "acoustic_grand_piano" | "orchestral_harp" | "violin" | "glockenspiel" | "church_organ";
const soundFontInstrumentNames: string[] = ["acoustic_grand_piano", "orchestral_harp", "violin", "glockenspiel", "church_organ"];

interface SoundFontHandControl {
    instrument: any | null; 
    handGainNode: GainNode | null;
    activeNotePlayer: any | null; 
    currentMidiNote: number | null;
}

let leftSoundFontControl: SoundFontHandControl = { instrument: null, handGainNode: null, activeNotePlayer: null, currentMidiNote: null }; // Path A: Controlled by Physical Right Hand
let rightSoundFontControl: SoundFontHandControl = { instrument: null, handGainNode: null, activeNotePlayer: null, currentMidiNote: null }; // Path B: Controlled by Physical Left Hand

// For User Custom Samples
const USER_CUSTOM_SAMPLE_ID = "user_custom_sample";
let customSampleBufferPathA: AudioBuffer | null = null; // Audio Path A (Physical Right Hand)
let customSampleNamePathA: string | null = null;
let customSampleBufferPathB: AudioBuffer | null = null; // Audio Path B (Physical Left Hand)
let customSampleNamePathB: string | null = null;
...
let standardBellSampleBuffer: AudioBuffer | null = null;
...
const WRIST = 0;
const FINGER_TIPS_INDICES = { // 0-based MediaPipe landmark indices for finger tips
...
let currentPlaybackMode: PlaybackMode = "theremin"; 
...
// User-defined custom landmark pairs
type UserDefinedLandmarkPair = [number, number]; // 0-based landmark indices
interface UserCustomPairs {
    [keyId: string]: UserDefinedLandmarkPair; // e.g., 'R-P': [0, 4]
}
let userCustomLandmarkPairs: UserCustomPairs = {};


// Tap Pad Mode specific constants and state
interface PadDefinition {
    id: string;
    rect: { x: number, y: number, w: number, h: number }; // Normalized coordinates
    midi: number;
    freq: number;
    color: string; // Color for drawing the pad
}
...
const FINGER_KEY_NOTES: Record<string, { midi: number, freq: number }> = {
...
// Potentially for custom sample names in future:
// const LOCALSTORAGE_KEY_CUSTOM_SAMPLE_NAME_A = 'customSampleNamePathA';
// const LOCALSTORAGE_KEY_CUSTOM_SAMPLE_NAME_B = 'customSampleNamePathB';


interface FingerKeyThresholds {
    [keyId: string]: number; // e.g., 'R-P': 70
}
let fingerKeyThresholds: FingerKeyThresholds = {};

interface FingerKeyMutedStates {
    [keyId: string]: boolean; // e.g., 'R-P': true (muted)
}
...
function isSoundFontInstrument(instrumentName: string): instrumentName is SoundFontInstrumentName {
    return soundFontInstrumentNames.includes(instrumentName);
}

async function ensureStandardBellSampleLoaded(audioCtx: AudioContext): Promise<boolean> {
    if (standardBellSampleBuffer) return true; // Already loaded
    if (standardBellSampleCurrentlyLoading) {
        // statusMessage.textContent = "Стандартный колокольчик всё ещё загружается..."; // Optional
        return false; 
    }
    if (standardBellSampleLoadingAttempted && !standardBellSampleBuffer) return false; // Failed before
...
function onResults(results: any) {
    if (!canvasCtx || !canvasElement || !audioContext) return;
            ...
            if (landmarks && landmarks[WRIST]) {
                ...
                FINGER_LABELS.forEach(fingerLabel => { 
                    ...
                    if (point1 && point2 && barFillElement) {
                        ...
                        if (valueDisplayElement) valueDisplayElement.textContent = '0%';
                         if (barFillElement) barFillElement.style.height = '0%';
                    }
                });
                ...
                if (currentPlaybackMode === "theremin") {
                    ...
                    if (handedness === 'Left') {  // Physical Left Hand (circle visual) -> controls 'Path B' (rightOscillator etc.)
                        ...
                        if (instrumentType === USER_CUSTOM_SAMPLE_ID && customSampleBufferPathB && audioContext && rightOscillatorGainNode) {
                            ...
                            if (rightSoundFontControl.currentMidiNote !== safeMidiNote) {
                                if (rightSoundFontControl.activeNotePlayer) rightSoundFontControl.activeNotePlayer.stop(now);
                        ...
                        if (instrumentType === USER_CUSTOM_SAMPLE_ID && customSampleBufferPathA && audioContext && leftOscillatorGainNode) {
                            ...
                            leftSoundFontControl.handGainNode.gain.setTargetAtTime(thereminGain, now, 0.01);
                             if (leftSoundFontControl.currentMidiNote !== safeMidiNote) {
                                if (leftSoundFontControl.activeNotePlayer) leftSoundFontControl.activeNotePlayer.stop(now);
                    ...
                    let targetPads: PadDefinition[] = [];
                    let instrumentValue: string = "";
                    let normVolumeX: number;
                    let currentOsc: OscillatorNode | null = null; 
                    let currentOscGain: GainNode | null = null; 
                    let currentSfControl: SoundFontHandControl | null = null; 
                    let currentCustomSampleBuffer: AudioBuffer | null = null;
                    // For Tap Pad, standard bell is also an option
                    let currentStandardBellBuffer: AudioBuffer | null = standardBellSampleBuffer;
                    ...
                    if (handedness === 'Right') { // Physical Right Hand (raw X is large, 0.5-1.0) -> controls 'Path A'
                        ...
                        currentOsc = leftOscillator; currentOscGain = leftOscillatorGainNode; currentSfControl = leftSoundFontControl;
                        ...
                        currentOsc = rightOscillator; currentOscGain = rightOscillatorGainNode; currentSfControl = rightSoundFontControl;
                    ...
                    for (const pad of targetPads) {
                        const tipX = indexFingerTipLandmark.x; const tipY = indexFingerTipLandmark.y;
                    ...
                    let instrumentValue: string;
                    let currentOsc: OscillatorNode | null, currentOscGain: GainNode | null, currentSfControl: SoundFontHandControl | null;
                    let currentCustomSampleBuffer: AudioBuffer | null = null;
                    let currentStandardBellBuffer: AudioBuffer | null = standardBellSampleBuffer;
                    let normVolumeX: number;
                    ...
                    FINGER_LABELS.forEach(fingerLabel => { 
                        const barFillElement = document.getElementById(`${handPrefixForDOM}-finger-${fingerLabel.toLowerCase()}-bar-fill`);
                        const valueDisplayElement = document.getElementById(`${handPrefixForDOM}-finger-${fingerLabel.toLowerCase()}-bar-value`) as HTMLElement | null;
                        ...
                        if (!isMuted && barHeightPercent < currentThreshold && state && !state.isActive && noteInfo) { 
                            ...
                            if (barFillElement) { 
                                ...
                                setTimeout(() => { if (barFillElement) barFillElement.style.backgroundColor = originalColor; }, 150);
                            }
    ...
    if (audioContext) { 
        if (currentPlaybackMode === "theremin") {
            if (!physLeftHandAudioControlledThisFrame) { 
                if (rightOscillatorGainNode) rightOscillatorGainNode.gain.setTargetAtTime(0, now, 0.1);
                if (rightSoundFontControl.handGainNode) rightSoundFontControl.handGainNode.gain.setTargetAtTime(0, now, 0.1);
                if (rightSoundFontControl.activeNotePlayer) { rightSoundFontControl.activeNotePlayer.stop(now + 0.1); rightSoundFontControl.activeNotePlayer = null; rightSoundFontControl.currentMidiNote = null; }
                isPathBCustomSamplePlaying = false; 
                isPathBStandardBellPlaying = false;
            }
            if (!physRightHandAudioControlledThisFrame) { 
                if (leftOscillatorGainNode) leftOscillatorGainNode.gain.setTargetAtTime(0, now, 0.1);
                if (leftSoundFontControl.handGainNode) leftSoundFontControl.handGainNode.gain.setTargetAtTime(0, now, 0.1);
                if (leftSoundFontControl.activeNotePlayer) { leftSoundFontControl.activeNotePlayer.stop(now + 0.1); leftSoundFontControl.activeNotePlayer = null; leftSoundFontControl.currentMidiNote = null; }
...
async function initializeSoundSource(
    instrumentNameParam: string,
    soundFontControlTarget: SoundFontHandControl,
    oscillatorTarget: OscillatorNode | null, // This will be the specific oscillator for the path (e.g. leftOscillator)
    gainNodeTarget: GainNode | null, // This will be the specific gain node for the path (e.g. leftOscillatorGainNode)
    physicalHandLabelForMessage: 'Left' | 'Right' // For status messages
): Promise<{ oscillator: OscillatorNode | null, gainNode: GainNode | null, soundFontControl: SoundFontHandControl }> {

    if (!audioContext) throw new Error("AudioContext not initialized");

    // Clear previous state of the target audio path
    if (oscillatorTarget) { oscillatorTarget.stop(); oscillatorTarget.disconnect(); oscillatorTarget = null; }
    if (gainNodeTarget) { gainNodeTarget.disconnect(); gainNodeTarget = null; }


    if (soundFontControlTarget.instrument && soundFontControlTarget.activeNotePlayer) { soundFontControlTarget.activeNotePlayer.stop(); soundFontControlTarget.activeNotePlayer = null; }
    ...
    let newOscillator: OscillatorNode | null = null;
    let newGainNode: GainNode | null = null;
    let newSoundFontControlState: SoundFontHandControl = {
        instrument: null,
        handGainNode: null,
    ...
    if (currentInstrumentName === USER_CUSTOM_SAMPLE_ID) {
        ...
        try {
            const sfInstrument = await Soundfont.instrument(audioContext, currentInstrumentName as SoundFontInstrumentName, { gain: 1 });
    ...
    if (isDirectOscillatorType || prepareFallbackOscillator) {
        ...
        let finalOscillatorType: OscillatorType = 'sine'; // Default for fallback
        ...
        if (prepareFallbackOscillator) {
            ...
            finalOscillatorType = currentInstrumentName as OscillatorType;
        }
...
function stopMusicInternal() { 
    if (leftOscillator) { leftOscillator.stop(); leftOscillator.disconnect(); leftOscillator = null; }
    if (leftOscillatorGainNode) { leftOscillatorGainNode.disconnect(); leftOscillatorGainNode = null; }
    if (rightOscillator) { rightOscillator.stop(); rightOscillator.disconnect(); rightOscillator = null; }
    if (rightOscillatorGainNode) { rightOscillatorGainNode.disconnect(); rightOscillatorGainNode = null; }

    if (leftSoundFontControl.activeNotePlayer) { leftSoundFontControl.activeNotePlayer.stop(); leftSoundFontControl.activeNotePlayer = null; }
    if (leftSoundFontControl.handGainNode) { leftSoundFontControl.handGainNode.disconnect(); }
    leftSoundFontControl.instrument = null; leftSoundFontControl.currentMidiNote = null;  leftSoundFontControl.handGainNode = null;

    if (rightSoundFontControl.activeNotePlayer) { rightSoundFontControl.activeNotePlayer.stop(); rightSoundFontControl.activeNotePlayer = null; }
    if (rightSoundFontControl.handGainNode) { rightSoundFontControl.handGainNode.disconnect(); }
    rightSoundFontControl.instrument = null; rightSoundFontControl.currentMidiNote = null; rightSoundFontControl.handGainNode = null;

    Object.keys(padInteractionStates).forEach(key => {
        if(padInteractionStates[key]) padInteractionStates[key].isActive = false;
    });
    Object.keys(fingerKeyStates).forEach(key => {
        if(fingerKeyStates[key]) fingerKeyStates[key].isActive = false;
    });
...
function setAppControlsDisabled(disabled: boolean) {
    ...
    if (leftCustomSampleInput) leftCustomSampleInput.disabled = disabled;
    if (rightCustomSampleInput) rightCustomSampleInput.disabled = disabled;

    document.querySelectorAll('input[name="playback-mode"]').forEach(radio => (radio as HTMLInputElement).disabled = disabled);
    document.querySelectorAll('input[name="distance-mode"]').forEach(radio => (radio as HTMLInputElement).disabled = disabled);
    if (infoButton) infoButton.disabled = disabled;

    ALL_FINGER_KEY_IDS.forEach(keyId => {
        const point1Select = document.getElementById(`${keyId.toLowerCase()}-point1-select`) as HTMLSelectElement | null;
        const point2Select = document.getElementById(`${keyId.toLowerCase()}-point2-select`) as HTMLSelectElement | null;
        if (point1Select) point1Select.disabled = disabled;
        if (point2Select) point2Select.disabled = disabled;
    });
}
...
async function startMusic() {
    ...
    try {
        ...
        statusMessage.textContent = `Ошибка: ${(err as Error).message}. Проверьте разрешения.`;
        stopMusicInternal(); 
        isTracking = false; 
        setAppControlsDisabled(false); 
        updateCustomPairsUIConfigVisibility();
...
controlButton.addEventListener('click', () => {
    if (!audioContext || audioContext.state === 'suspended') {
         ...
         audioContext.resume().then(() => {
            if (isTracking) stopMusic(); else startMusic();
         }).catch(e => {
            statusMessage.textContent = "Не удалось инициализировать аудио. Взаимодействуйте со страницей."; console.error("AudioContext resume failed:", e);
         });
    } else {
        if (isTracking) stopMusic(); else startMusic();
    }
...
async function handleInstrumentChange(physicalHand: 'Left' | 'Right') { 
    ...
    if (instrumentName === USER_CUSTOM_SAMPLE_ID) {
        ...
        if (fileInput) fileInput.click(); // Triggers file selection dialog
...
function createFileInputHandler(physicalHand: 'Left' | 'Right'): (event: Event) => Promise<void> {
    return async (event: Event) => {
        ...
        if (!audioContext) { // Double check after attempt
             statusMessage.textContent = "Ошибка: Аудиоконтекст не доступен."; return;
        }
            ...
            try {
                ...
                if (isTracking) {
                    if (isLeftHand) {
                        ...
                        rightOscillator = sources.oscillator; rightOscillatorGainNode = sources.gainNode; rightSoundFontControl = sources.soundFontControl;
                    } else {
                        ...
                        leftOscillator = sources.oscillator; leftOscillatorGainNode = sources.gainNode; leftSoundFontControl = sources.soundFontControl;
                    }
                }
            } catch (e) {
                ...
                if (isLeftHand) { customSampleBufferPathB = null; customSampleNamePathB = null; }
                else { customSampleBufferPathA = null; customSampleNamePathA = null; }
                ...
                if (isTracking) {
                    ...
                    if (selectEl.value === USER_CUSTOM_SAMPLE_ID) selectEl.value = 'sine';
                }
            } finally {
                setAppControlsDisabled(false);
                updateCustomPairsUIConfigVisibility();
...
if (leftCustomSampleInput) leftCustomSampleInput.addEventListener('change', createFileInputHandler('Left'));
if (rightCustomSampleInput) rightCustomSampleInput.addEventListener('change', createFileInputHandler('Right'));
...
document.querySelectorAll('input[name="playback-mode"]').forEach(radio => {
    radio.addEventListener('change', (event) => {
        currentPlaybackMode = (event.target as HTMLInputElement).value as PlaybackMode;
        ...
        let modeName = "";
        if (currentPlaybackMode === 'theremin') modeName = "Ручной терменвокс";
        else if (currentPlaybackMode === 'tappad') modeName = "Сенсорные пэды";
        else if (currentPlaybackMode === 'fingerkeys') modeName = "Пальцевые клавиши";
        statusMessage.textContent = `Режим изменен на: ${modeName}.`;

        if (isTracking && audioContext) { 
            if (leftOscillatorGainNode) leftOscillatorGainNode.gain.setTargetAtTime(0, audioContext.currentTime, 0.01); 
            if (rightOscillatorGainNode) rightOscillatorGainNode.gain.setTargetAtTime(0, audioContext.currentTime, 0.01); 
            
            if (leftSoundFontControl.activeNotePlayer) { leftSoundFontControl.activeNotePlayer.stop(); leftSoundFontControl.activeNotePlayer = null; leftSoundFontControl.currentMidiNote = null;} 
            if (leftSoundFontControl.handGainNode) leftSoundFontControl.handGainNode.gain.setTargetAtTime(0, audioContext.currentTime, 0.01); 
            
            if (rightSoundFontControl.activeNotePlayer) { rightSoundFontControl.activeNotePlayer.stop(); rightSoundFontControl.activeNotePlayer = null; rightSoundFontControl.currentMidiNote = null;} 
            if (rightSoundFontControl.handGainNode) rightSoundFontControl.handGainNode.gain.setTargetAtTime(0, audioContext.currentTime, 0.01); 
            
            Object.keys(padInteractionStates).forEach(key => { if(padInteractionStates[key]) padInteractionStates[key].isActive = false; });
            Object.keys(fingerKeyStates).forEach(key => { if(fingerKeyStates[key]) fingerKeyStates[key].isActive = false; });
            isPathACustomSamplePlaying = false; 
            isPathBCustomSamplePlaying = false; 
            isPathAStandardBellPlaying = false;
            isPathBStandardBellPlaying = false;
...
document.querySelectorAll('input[name="distance-mode"]').forEach(radio => {
    radio.addEventListener('change', (event) => {
        currentDistanceMode = (event.target as HTMLInputElement).value as DistanceCalculationMode;
        updateCustomPairsUIConfigVisibility();
        try {
        ...
        let modeName = "";
        if (currentDistanceMode === 'wrist_tip') modeName = "От запястья до кончика";
        else if (currentDistanceMode === 'custom_points') modeName = "Между указанными точками";
        else if (currentDistanceMode === 'custom_user_defined') modeName = "Кастомный (пользовательский)";
...
if (canvasElement) {
    ...
    if (videoContainer) initialWidth = videoContainer.offsetWidth;
    else if (window.innerWidth < 500) initialWidth = window.innerWidth * 0.9;
    
    canvasElement.width = initialWidth;
    canvasElement.height = initialWidth / aspectRatio;

    const eqContainer = document.querySelector('.equalizer-container') as HTMLElement | null;
    if (eqContainer) {
...
function updateThresholdDisplay(fingerKeyId: string) {
...
function adjustThreshold(fingerKeyId: string, increase: boolean) {
...
function updateMuteVisual(fingerKeyId: string) {
...
function toggleMuteState(fingerKeyId: string) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(leftCustomSampleNameDisplay) leftCustomSampleNameDisplay.textContent = "Нет файла";
    if(rightCustomSampleNameDisplay) rightCustomSampleNameDisplay.textContent = "Нет файла";
    ...
    if (savedThresholdsString) {
        try {
            const loadedThresholds = JSON.parse(savedThresholdsString) as FingerKeyThresholds;
    ...
    if (savedMutedStatesString) {
        try {
            const loadedMutedStates = JSON.parse(savedMutedStatesString) as FingerKeyMutedStates;
    ...
    if (savedCustomPairsString) {
        try {
            const loadedPairs = JSON.parse(savedCustomPairsString) as UserCustomPairs;
    ...
    if (!customPairsLoadedSuccessfully) {
        ...
        ALL_FINGER_KEY_IDS.forEach(keyId => {
            const fingerChar = keyId.substring(2) as keyof typeof FINGER_TIPS_INDICES; 
        ...
        } catch (e) { console.error("Error saving initial default custom pairs:", e); }
    }

    ALL_FINGER_KEY_IDS.forEach(keyId => {
        for (let pointIndex = 0; pointIndex < 2; pointIndex++) {
            const selectElement = document.getElementById(`${keyId.toLowerCase()}-point${pointIndex + 1}-select`) as HTMLSelectElement | null;
            if (selectElement) {
                ...
                selectElement.addEventListener('change', (event) => {
                    const changedSelect = event.target as HTMLSelectElement;
    ...
    if (savedLeftInstrument && leftInstrumentSelect) {
        ...
        if (optionExists) leftInstrumentSelect.value = savedLeftInstrument;
        else console.warn(`Saved instrument "${savedLeftInstrument}" for circle hand not found in select options.`);
    }
    ...
    if (savedRightInstrument && rightInstrumentSelect) {
        ...
        if (optionExists) rightInstrumentSelect.value = savedRightInstrument;
        else console.warn(`Saved instrument "${savedRightInstrument}" for triangle hand not found in select options.`);
    }

    const savedPlaybackMode = localStorage.getItem(LOCALSTORAGE_KEY_PLAYBACK_MODE) as PlaybackMode | null;
    if (savedPlaybackMode) {
        const radioElement = document.getElementById(`mode-${savedPlaybackMode}`) as HTMLInputElement | null;
        if (radioElement) {
            ...
            try { 
                localStorage.setItem(LOCALSTORAGE_KEY_PLAYBACK_MODE, currentPlaybackMode);
            } catch (e) { console.error("Error saving default playback mode to localStorage:", e); }
        }
    }
    updatePlaybackModeVisuals(); 

    const savedDistanceMode = localStorage.getItem(LOCALSTORAGE_KEY_DISTANCE_MODE) as DistanceCalculationMode | null;
    if (savedDistanceMode) {
        const radioIdSuffix = savedDistanceMode.replace(/_/g, '-');
        const radioElement = document.getElementById(`mode-dist-${radioIdSuffix}`) as HTMLInputElement | null;
        ...
        if (radioElement) {
            ...
            console.warn(`Saved distance mode "${savedDistanceMode}" (ID: mode-dist-${radioIdSuffix}) not found. Reverting to default.`);
             try { 
                localStorage.setItem(LOCALSTORAGE_KEY_DISTANCE_MODE, currentDistanceMode);
            } catch (e) { console.error("Error saving default distance mode to localStorage:", e); }
        }
    }
    updateCustomPairsUIConfigVisibility(); 
